...
NIC_DETAILS=$(
  ...
  --query "[].{NicId:id, PrivateIP:ipConfigurations[0].privateIPAddress, PublicIPId:ipConfigurations[0].publicIPAddress.id}" \
  --output json
)
...
PUBLIC_IP_DETAILS=$(
  ...
  --resource-group ${RESOURCE_GROUP} \
  --query "[].{PublicIPId:id, PublicIPAddress:ipAddress}" \
  --output json
)
